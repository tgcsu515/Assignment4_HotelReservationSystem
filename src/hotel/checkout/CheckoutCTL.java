...
public class CheckoutCTL {
    ...
    public String getAllServiceChargesRecorded(Booking currentBooking) {
        StringBuilder sb = new StringBuilder();
        total = 0;
        String message = null;
        try {
            List<ServiceCharge> charges = currentBooking.getCharges();
            for (ServiceCharge sc : charges) {
                total += sc.getCost();
                String chargeStr = String.format("    %-12s:%10s",
                        sc.getDescription(), String.format("$%.2f", sc.getCost()));
            ...
            String mesg = String.format("CheckoutCTL: roomIdEntered :" + ex.getMessage());
            throw new RuntimeException(mesg);
        }
        return message;
    }
